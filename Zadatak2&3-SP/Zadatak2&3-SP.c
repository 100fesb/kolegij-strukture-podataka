...
Pozicija traziPrez(Pozicija);		//protorip funckije koja prima pokazivac na strukturu a vraca pokazivac na trazenu
...
int sortiraj(Pozicija);		//prototip funckije koja sortira od najmanjeg pocetnog prezimena do najveceg,NE UCI OVU	
...
int main()		//main koja ne prima nista
{
	...
	for (i = 0; i < broj_el; i++)		//for se izvrti onoliko puta koliko smo htjeli unijeti osoba
	{
	...
	for (i = 0; i < broj_el; i++)		//for se izvrti onoliko puta koliko smo htjeli unijeti osoba
	{
		...
		ispis(Head.Next);		//pozivamo funkciju za ispis i saljemo joj pokazivac iz Head-a na prvi element
	}
	...
	printf("Unosimo ispred:");		//printf obavijest
	test = traziPrez(&Head);		//pozivamo funckiju traziPrez i ona vraca pokazivac na onu strukturu u kojoj se nalazi prezime koje smo trazili
	unosispred(test,&Head);		//funckija prima pokazivac na element liste ispred kojega upisujemo i adresu Head-a
	ispis(Head.Next);		//pozivamo funckiju za ispis i saljemo joj pokazivac iz Head-a dalje jer u Head-u nema osoba to je samo prvi element liste

	printf("Unosimo iza:");		//printf obavijest
	test = traziPrez(&Head);		//pozivamo funckiju traziPrez i ona vraca pokazivac na onu strukturu u kojoj se nalazi prezime koje smo trazili
	unosiza(test, &Head);		//funckija prima pokazivac na element liste iza kojeg upisujemo i na Head
	ispis(Head.Next);		//pozivamo funckiju za ispis i saljemo joj pokazivac iz Head-a dalje jer u Head-u nema osoba to je samo prvi element liste

	printf("Sortiramo:");		//printf obavijest
	sortiraj(&Head);		//funckija sortira listu po prezimenima a prima adresu Head-a
	ispis(Head.Next);		//pozivamo funckiju za ispis i saljemo joj pokazivac iz Head-a dalje jer u Head-u nema osoba to je samo prvi element liste
	*/
	upisujeuFile(&Head);		//upisujemo listu u File a funkcija prima adresu Head-a
	ispis(Head.Next);		//ispis liste ako je u funkciji upisujeuFile proslo sve dobro

	citaizFile(&Head);		//citamo listu iz File-a a funkcija prima adresu Head-a
	ispis(Head.Next);		//ispis liste ako je u funkciji citaizFile proslo sve dobro
...
int unosiza(Pozicija unesiiza, Pozicija P)		//funkcija prima pokazivac na element iza kojeg upisujemo i adresu Head-a
{
	...
	else {		//izvrsava se ako nije pronadeno prezime iako mora bit jer smo to provjerili u funkciji traziPrez
		printf("Nema tog prezimena!");		//printf obavijest
	}
...
int citaizFile(Pozicija P) {		//prima adresu na Head
	...
	buffer = (char*)malloc(sizeof(char) * N_LENGTH);		//dinamicka alokacija za buffer

	test = (Pozicija)malloc(sizeof(struct Cvor));		//dinamicka alokacija za test

	if (!fileName)  printf("Greska prilikom alokacije");		//provjera jeli se pohranilo ime datoteke
