/*
11. Napisati kod koji za zadane podatke studenata (matièni broj, ime i prezime) pravi
...
struct hashTablica {
	int velicinaTablice;
	Lista* hashLista;		//pokazivac na pokazivac jer je HashTablica polje pokazivaca na strukture tipa Student
};		
...
int CitajIzDat(Pozicija P) {
	...
	while (!feof(fp)) {		// kao i ranije, stvaramo vezanu listu svih studenata, generiranu iz datoteke
		q = StvoriNovogS();
...
int UpisuHashTablicu(Pozicija P, hashTab H)
{
	...
	while (P != NULL) {
		...
		if (H->hashLista[kljuc] != NULL)		// ako vec postoji student s istim kljucem - radi novu vezanu listu
			H->hashLista[kljuc]->Next = P;
		else
			H->hashLista[kljuc] = P;

		temp = StvoriNovogS();		//temp objasnjen gore
		temp = P;
		P = P->Next;
		temp->Next = NULL;
	}
